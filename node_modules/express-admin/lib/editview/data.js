...
var otm = {
	_getRef: function (args, ref, cb) {
		...
		var str = query.replace('get-records', [pk,text].join(), sql.table(ref.table));
...
var tbl = {
	_getSql: function (args) {
		...
		return query.replace('get-record',
			names.join(), sql.table(table.name), sql.andValues(table.name, (args.fk? args.fk:table.pk), args.id));
	},
	// public - add docs
	get: function (args, cb) {
		if (args.post) {
...
var mtm = {
	...
	_getIds: function (column, pk, args, cb) {
		...
		var str = query.replace('get-record',
			sql.concat(link.table, link.childPk), sql.table(link.table), sql.andValues(link.table, link.parentPk, pk));
